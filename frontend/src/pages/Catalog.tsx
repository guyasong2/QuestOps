...
export default function Catalog() {
  ...
  if (loading) {
    return <div className="min-h-screen bg-bg flex items-center justify-center text-text-muted font-mono">Loading Quest Catalog...</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-bg flex flex-col text-text font-body">
      {/* Top Navigation Bar */}
      <header className="h-16 border-b border-border bg-surface flex items-center px-6 justify-between sticky top-0 z-20 shadow-sm">
        <div className="flex items-center gap-3">
          <img src="/QuestOps_logo.png" alt="QuestOps" className="h-8 object-contain" />
          <span className="font-display font-bold text-xl tracking-tight hidden sm:block">The QuestOps Catalog</span>
        </div>
        
        <div className="flex items-center gap-6">
          <div className="hidden md:flex items-center gap-2 text-sm text-text-muted cursor-help">
            <span>💡</span> Pssssst! Need a hint?
          </div>
          ...
          <div className="flex items-center gap-3 border-l border-border pl-6">
            ...
            <button 
              ...
              className="w-8 h-8 rounded bg-blue-500 hover:bg-blue-600 text-white flex items-center justify-center font-bold uppercase transition-colors shadow-sm"
              title="Logout"
            >
              {user?.username?.[0] || 'U'}
            </button>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-16 border-r border-border bg-surface flex flex-col items-center py-6 gap-6 z-10 shrink-0">
          <button className="text-text-muted hover:text-text transition-colors p-2" title="Home">
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>
          </button>
          <button className="text-blue-600 bg-blue-50 border border-blue-100 p-2 rounded-lg" title="Catalog">
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg>
          </button>
          <button className="text-text-muted hover:text-text transition-colors p-2" title="Favorites">
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" /></svg>
          </button>
          <button className="text-text-muted hover:text-text transition-colors p-2" title="Community">
        ...
        <main className="flex-1 overflow-y-auto p-6 md:p-8">
          <div className="max-w-6xl mx-auto space-y-10">
            {tracks.map((track) => {
              if (track.scenarios.length === 0) return null;
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {track.scenarios.map((scenario, idx) => (
                      <motion.div 
                        ...
                        className="bg-surface border border-border rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow cursor-pointer group flex flex-col h-full"
                      >
                        ...
                        >
                          ...
                          <h3 className="text-2xl font-display font-black text-gray-900 text-center uppercase tracking-widest drop-shadow-sm relative z-10 truncate px-4">
                            {scenario.title}
                          </h3>
                        </div>
                        ...
                        <div className="p-5 flex flex-col flex-1">
                          ...
                          <div className="flex gap-2 flex-wrap mt-auto">
                            <span className="text-xs font-mono px-2 py-1 border border-border rounded text-text-muted bg-bg">
                              {track.name}
                            </span>
